...
pub async fn search(State(state): State<AppState>, Json(body): Json<serde_json::Value>) -> Json<serde_json::Value> {
    ...
    let url = format!("{}/ai/search", state.kubi_ai_url);
    match state.http_client.post(&url).json(&body).send().await {
        ...
        Err(e) => Json(json!({"error": "Search failed", "message": e.to_string()})),
    }
}
